...
var rotateLeft = function(pNode) {
    ...
    var tempNode = pNode.right;
    
    pNode.right = tempNode.left;                        // Assign left child of temporary node as right child of current node
    tempNode.left = pNode;                              // Assign the current node to be left child of the temporary node
    tempNode.color = pNode.color;                       // Set color of temporary node to be same as current node
    pNode.color = RedBlackTree._ID_NODE_COLOR_RED;      // Set color of current node to be "Red"
...
var rotateRight = function(pNode) {
    ...
    var tempNode = pNode.left;
    
    pNode.left = tempNode.right;                        // Assign right child to temporary node as left child of current node
    tempNode.right = pNode;                             // Assign the current node to be right child of the temporary node
    tempNode.color = pNode.color;                       // Set color of temporary node to be same as current node
    pNode.color = RedBlackTree._ID_NODE_COLOR_RED;      // Set color of current node to be "Red"
...
var flipColors = function(pNode) {
    ...
    pNode.right.flipColor();
    pNode.left.flipColor();
...
var fixNode = function(pNode) {
    ...
    if (RedBlackNode.isRed(pNode.right) && !RedBlackNode.isRed(pNode.left)) {
    ...
    if (RedBlackNode.isRed(pNode.left) && RedBlackNode.isRed(pNode.left.left)) {
    ...
    if (RedBlackNode.isRed(pNode.left) && RedBlackNode.isRed(pNode.right)) {
...
var moveLeft = function(pNode) {
    ...
    if (RedBlackNode.isRed(pNode.right.left)) {
...
var moveRight = function(pNode) {
    ...
    if (RedBlackNode.isRed(pNode.left.left)) {
        // If the left child of current node's left child is Red
...
var removeMinimum = function(pNode) {
    ...
    if (typeof pNode.left === "undefined") {
        return undefined;
    }
    ...
    pNode.left = removeMinimum(pNode.left);
...
var remove = function(pNode, pData, pTree) {
    ...
    if (result > 0) {
        ...
        if (result === 0) {
            ...
            pNode.right = removeMinimum(pNode.right);
...
RedBlackTree._ID_TRAVERSE_PRE_ORDER   = BinaryTreeIterator._ID_TRAVERSE_PRE_ORDER;
RedBlackTree._ID_TRAVERSE_IN_ORDER    = BinaryTreeIterator._ID_TRAVERSE_IN_ORDER;
RedBlackTree._ID_TRAVERSE_POST_ORDER  = BinaryTreeIterator._ID_TRAVERSE_POST_ORDER;
