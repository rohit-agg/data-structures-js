...
var swim = function(pTree, pIndex, pHeap) {
    ...
    while (pIndex > 0 && pHeap.compare(pTree[pIndex], pTree[(pIndex-1)/2]) > 0) {
...
MaxHeap.prototype.remove = function(pData) {
    ...
    while (searchIndex < this.size) {
        ...
        if (result == 0) {
            // If the comparison is found to be equal, node is found
 ...
 *  @function           removeMax
 *  @class              MaxHeap
 *  @return             Object          Object of the data present on the top of Heap as Maximum value
 *  @description
 *  <ul>
 *      <li>Data to be deleted is exchnaged with the last node</li>
 *      <li>The new root of the tree is sinked to place it in correct position</li>
 *  </ul>
 */
MaxHeap.prototype.removeMax = function() {
    
    maxOfHeap = this.size-1;            // Store the current Max of Heap into a variable
    
    // Exchange the data in the current node with the last node in the tree
    exchange(this.values, 0, this.size-1);
    
    delete this.values[this.size-1];            // Delete the data present on the last node
    this.size--;                                // Decrement the size of the heap
    sink(this.values, 0, this);                 // Re-adjust the heap to satisfy max-heap property
    
    return maxOfHeap;                   // Return the value that was present as Max of Heap
}
